...
class AttentiveLayer(nn.Module):
    def __init__(self, u_fea, i_fea):
        super(AttentiveLayer, self).__init__()
        
        related_pw = torch.tanh(torch.dot(u_fea, i_fea))
        u_context_fea = torch.mean(related_pw)
        i_context_fea = torch.mean(torch.transpose(related_pw))
