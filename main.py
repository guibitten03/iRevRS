...
def train(**kwargs):
    ...
    # getattr(objeto, atributo que quero pegar)
...
# from metrics.micro_metrics import diversity
from metrics.catalog_coverage import catalog_coverage
from metrics.distributional_coverage import *

def calculate_metrics(true, pred, test_data, output):
    mse = mean_squared_error(true, pred)
    mae = mean_absolute_error(true, pred)
    f1 = f1_score(true, pred, average="macro")
    acc= accuracy_score(true, pred)
    sur = serendipity(true, pred, 4.0)
    div = diversity(true, pred, 4.0)
    
    # train_df = pd.DataFrame({
    #     "userID": test_data[2][:len(test_data)].tolist(),
    #     "itemID": test_data[3][:len(test_data)].tolist()
    # })
    # reco_df = pd.DataFrame({
    #     "userID": test_data[2][len(test_data):].tolist(),
    #     "itemID": test_data[3][len(test_data):].tolist(),
    #     "col_relevance": output[len(test_data):]
    # })
    
    # diver = catalog_coverage(output.view(len(output), 1), test_data[3].view(len(test_data[3]), 1).to(torch.int64), )
    # diver = distributional_coverage(output.view(len(output), 1), test_data[3].view(len(test_data[3]), 1).to(torch.int64), k=3)
    
    # diver = diversity(train_df, reco_df)
    
    return mse, mae, f1, acc, sur, div
...
def predict(model, data_loader, opt):
    ...
    with torch.no_grad():
        for idx, (test_data, scores) in enumerate(data_loader):
            ...
            mse, mae, acc, f1, sur, div = calculate_metrics(list_scores, list_out, test_data, output)
            
            if True:
                df = pd.read_csv("results.csv")
                df.loc[df.shape[0]] = [mse, mae, acc, f1, sur, div]
...
def unpack_input(opt, x):
    ...
    # Aqui ele pega as informaçoes dos ids
    # Pega os usuários/itens correspondentes e informações
