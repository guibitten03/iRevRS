...
# Have to change in code for get datasets
parser.add_argument("-d", "--dataset", type = str, default = "Philadelphia_data", help = "Dataset (Default: amazon_instant_video)")
...
train_df = pd.read_csv("../../dataset/Video_Ga_data/train/Train.csv")
test_df = pd.read_csv("../../dataset/Video_Ga_data/test/Test.csv")
val_df = pd.read_csv("../../dataset/Video_Ga_data/val/Val.csv")
...
def split_rows(df):
    ...
    for row in df.values:
        ...
        date = 1
        text = str(row[3])
        inter.append([user, item])
    
    return inter

train_interactions = split_rows(train_df)
dev_interactions = split_rows(val_df)
test_interactions = split_rows(test_df)

interactions = train_interactions + dev_interactions + test_interactions
...
def split_rows(df):
    ...
    for row in df.values:
        ...
        rating = row[2]
        date = 1
        text = simple_tokenizer(str(row[3]))
        inter.append([user, item, rating, date, text])
...
# 				text = simple_tokenizer(d['reviews'])
# 			else:
# 				rating = d['overall']
