...
parser.add_argument("-e", 			dest = "epochs", 		type = int, 	default = 10, 			help = "Number of Training Epochs (Default: 25)")
...
def evaluate(mdl, set_loader, epoch_num = -1, use_cuda = True, phase = "Dev", print_txt = True):
	...
	for batch_num, (batch_uid, batch_iid, batch_rating) in enumerate(set_loader):
		...
		list_out = torch.round(rating_pred).int()
		list_scores = torch.round(batch_rating).int()
		...
		sur = serendipity(list_scores, list_out, threshold=4.0)
		div = diversity(list_scores, list_out)
  
		MSE = mean_squared_error(list_scores, list_out)
		MAE = mean_absolute_error(list_scores, list_out)
  
		if phase == "Test":
			results = pd.read_csv("results.csv")
			results.loc[results.shape[0]] = [MSE,MAE,acc,f1,sur,div]
	...
	print(
        f"\tevaluation result: ACC : {acc:.4f}; MSE: {MSE:.4f}; RMSE: {math.sqrt(MSE):.4f}; MAE: {MAE:.4f}; F1: {f1:.4f}; Sur: {sur:.4f}; Div: {div:.4f}"
        )
