    ...
    with tf.Session() as sess:
        ...
        for e in range(epochs):
            ...
            for i in range(int(math.ceil(len(user_input) / float(batch_size)))):
                ...
                count += 1.0
            t1 = time()
            val_mses, val_maes, acc, pre, rec, f1, ser, div = [], [],[],[],[],[],[],[]
            for i in range(len(user_input_val)):
                rmses, maes, f1, acc, ser, div = eval_model(users, items, users_inputs, items_inputs, dropout_rate, predict_rating, sess, user_vals[i],
        ...
        for i in range(len(user_input_test)):
            mses, maes = [], []
            rmses, maes, f1, acc, ser, div = eval_model(users, items, users_inputs, items_inputs, dropout_rate, predict_rating, sess, user_tests[i], item_tests[i], user_input_test[i], item_input_test[i], rating_input_test[i], mses, maes)
            mse = np.array(rmses).mean()
            mae = np.array(maes).mean()
            
            results = pd.read_csv("CARL/runner/results.csv")
            
            results.loc[results.shape[0]] = [mse, mae, acc, f1, ser, div]
                    
            results.to_csv("CARL/runner/results.csv", index=False)
...
def eval_model(users, items, users_inputs, items_inputs, dropout_rate, predict_rating, sess, user_batch, item_batch, user_input_batch, item_input_batch, rate_tests, rmses, maes,):
    ...
    for r in range(row):
        true.append(rate_tests[r][0])
        pred.append(predicts[r,0])
        # mse = pow((predicts[r, 0] - rate_tests[r][0]), 2)
        # mae = abs((predicts[r, 0] - rate_tests[r][0]))
        rmses.append(pow((predicts[r, 0] - rate_tests[r][0]), 2))
        maes.append(abs((predicts[r, 0] - rate_tests[r][0])))
        
    true = list(map(lambda x: int(round(x)), true))
    ...
    ser = serendipity(true, pred, 4.0)
    div = diversity(true, pred)
    ...
    # print(f"Acc: {acc}; Pre: {precision}; Rec: {recall}; F1: {f1}")
        
    return rmses, maes, f1, acc, ser, div
...
if __name__ == "__main__":
    ...
    # loading data
    firTime = time()
    dataSet = Dataset(max_doc_length, "./carl_carp_parl/",
                      "WordDict.out")
