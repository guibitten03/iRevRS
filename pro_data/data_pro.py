...
# Cria um documento com as reviews
# Cria o tfidf para cada palavra para cada documento
...
def countNum(xDict):
    ...
    for (i, text) in xDict.items():
        # Máximo de reviews que tem no dataset de treino
        sumNum = sumNum + len(text)
        # Calcula o número minimo de reviews que as user/item tem
        if len(text) < minNum:
            minNum = len(text)
        # Calcula o numero maximo de reviews que as user/item tem
        if len(text) > maxNum:
            maxNum = len(text)
        # Cria uma lista com o tamanho de todos os documentos: Número de reviews/doc
        ReviewLenList.append(len(text))
        # Para cada review no documento
        for sent in text:
            # SentLenList.append(len(sent))
            # Separa as reviews em tokens
            if sent != "":
                wordTokens = sent.split()
            # Calcula a qtd máxima de palavras em uma rev
            if len(wordTokens) > maxSent:
                maxSent = len(wordTokens)
            # Calcula a qtd minima de palavras em uma rev
            if len(wordTokens) < minSent:
                minSent = len(wordTokens)
            # Cria uma lista com o tamanho de todas as reviews
            SentLenList.append(len(wordTokens))
            
    # Media de reviews por usuário
    averageNum = sumNum // (len(xDict))

    # Ordena a lista de tamanho de todas as reviews
    x = np.sort(SentLenList)
    # Calcula o tamanho da lista
    xLen = len(x)
    # Pega a palavra que ta no lugar do 85% da review
    # Ex: [5,6,7,8] -> Tamanho da review escolhida = 7
    pSentLen = x[int(P_REVIEW * xLen) - 1]
    x = np.sort(ReviewLenList)
    xLen = len(x)
    # Pega a review que ta no lugar da 85% do doc
...
if __name__ == '__main__':
    ...
    # python3 data_pro.py Philadelphia_data.csv yelp
    start_time = time.time()
    assert(len(sys.argv) >= 2)
    filename = sys.argv[1]

    yelp_data = False
    # save_folder = '../dataset/Yelp_data'
    
    if len(sys.argv) > 2 and sys.argv[2] == 'yelp':
        # yelp dataset
        yelp_data = True
        save_folder = '../dataset/' + filename[:-5]+"_data"
    else:
        # amazon dataset
    ...
    # Run dataset pro experimento: python3 data_pro.py Tucson_data.csv yelp > tuc_data_out.txt
    ...
    '''
        Transforma os ids dos usuários e itens em números
    '''

    # Constroi os dicionarios com os ids do usuário e seus numeros do dataframe
    ...
    '''
        Split dataset em treino, validação e teste
    '''
    ...
    # Pega o conjunto de todos os usuários
    ...
    '''
        Extração de informações do dataset
    '''
    
    '''
        Salvamento do dataset splitad para passar para outros códigos
    '''
    
    data.to_csv(f"{save_folder}/data.csv", index=False)
    data_train.to_csv(f"{save_folder}/train/Train.csv", index=False)
    data_test.to_csv(f"{save_folder}/test/Test.csv", index=False)
    data_val.to_csv(f"{save_folder}/val/Val.csv", index=False)

    # Extrai todos os pares de (user, item) -> Rating
    #                               x            y
    ...
    '''
        Construção do vocabulario e dados das reviews
    '''
    ...
    # User Reviews Dict
    # {user 1: ['review 1', 'review 2', 'review 3'...]}
    
    # User Iid Dict
    # {user 1: [Item consumido 1, Item consumido 2...]}
    
    # Extrai o vocabulario com TF-IDF
    ...
    # print("{} - {} - {} - " \
    #      "{} - {}，" \
    #      "{} - {}".format(u_minNum, u_maxNum, u_averageNum, u_maxSent, u_minSent, u_pReviewLen, u_pSentLen))
    ...
    # Pega a maior sequencia de palavras em uma sentença
    ...
    # Manda para a função os ids dos items que o user consumiu
    # O tamanho da lista de reviews que o user escreveu
    # Tamanho da quantidade de itens que tem no treino
    def padding_ids(iids, num, pad_id):
        # Se a quantidade de itens que o user consumiu é maior que a quantidade de itens
        # que o user revisou, então limita para a quantidade de itens que o user comentou
        if len(iids) >= num:
            new_iids = iids[:num]
        # Se não, preenche a quantidade de itens que o user comentou com nada
        else:
            new_iids = iids + [pad_id] * (num - len(iids))
            
        # Retorna os ids dos itens que o user consumiu
        return new_iids
    ...
    for i in range(userNum):
        ...
        # Pega os textos do usuário i
        textList = user_reviews_dict[i]
        
        # print(textList)
        
        # Pega os itens que o usuário i forneceu textos
        u_iids = user_iid_dict[i]
        u_reviewList = []

        # Manda para a função os ids dos items que o user consumiu
        # O tamanho da sentença máxima que um user escreveu
        # Tamanho da quantidade de itens que tem no treino
        user_iid_list.append(padding_ids(u_iids, u_pReviewLen, itemNum+1))
        # Transforma o documento do usuário que é feito com palavras em um documento
        # de indices de palavras
        doc2index = [word_index[w] for w in user_review2doc[i]]

        # Para cada review na lista de reviews do usuário
        for text in textList:
            text2index = []
            wordTokens = text.strip().split()
            # Separa os tokens de cada review
            if len(wordTokens) == 0:
                wordTokens = ['<unk>']
            # Transforma a review do usuário que é feita de palavras em uma review
            # de indices de palavras
            text2index = [word_index[w] for w in wordTokens]
            # Se o tamanho da review é menor que o tamanho da review maxima
            # Preenche com zeros
            if len(text2index) < maxSentLen:
                text2index = text2index + [0] * (maxSentLen - len(text2index))
            else:
                # Se for igual ou maior, limita o tamanho da review
    ...
    print(type(userReview2Index))
    print(len(userReview2Index))
    print(np.array(userReview2Index).shape)
    ...
    # Ordena o dicionario de index de palavras pelo index que a palavra recebeu
    vocab_item = sorted(word_index.items(), key=itemgetter(1))
    w2v = []
    out = 0
    
    # Transforma todas os documentos em word vectors com w2v
    if PRE_W2V_BIN_PATH:
