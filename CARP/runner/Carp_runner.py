...
from Evaluation import get_test_list_mask
epsilon = 1e-9
...
def train_model():
    ...
    #gate mechanism
    # Cria vetores de peso do contexto (viewpoint) de cada palavra
    # Cria portões de peso do contexto global (gates) de cada contexto
    # Cria uma matriz de bias
    # Tudo em referência a distribuição normal
    ...
    with tf.Session() as sess:
        ...
        for i in range(len(user_input_test)):
            ...
            rmses, maes, mse, mae, f1, acc, ser, div = eval_model(users, items, users_inputs, items_inputs, users_masks_inputs, items_masks_inputs,
            ...
            results = pd.read_csv("CARP/runner/results.csv")
            results.loc[results.shape[0]] = [mse, mae, acc, f1, ser, div]
                    
            results.to_csv("CARP/runner/results.csv", index=False)
...
def eval_model(users, items, users_inputs, items_inputs, users_masks_inputs, items_masks_inputs, dropout_rate,
    ...
    return rmses, maes, mse, mae, f1, acc, ser, div
