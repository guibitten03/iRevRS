...
def eval_model(users, items, users_inputs, items_inputs, users_aux_inputs, users_masks_inputs, items_masks_inputs, users_aux_masks_inputs, drop_out_rate, predict_rating, sess, user_test, item_test, user_input_test, item_input_test, user_aux_input_test, user_mask_input_test, item_mask_input_test, user_aux_mask_input_test, rating_input_test):
    ...
    for i in range(int(len(user_input_test) / 100 + 1)):
        ...
        for r in range(row):
            true.append(rates_batch_test[r])
            pred.append(predict[r,0])
    ...
    results = pd.read_csv("PARL/Runner/results.csv")
            
    results.loc[results.shape[0]] = [math.pow(rmse,2), rmse, mae, acc, pre, rec, f1, ser, div]
            
    results.to_csv("PARL/Runner/results.csv", index=False)
...
if __name__ == "__main__":
    ...
    # loading data
    firTime = time()
    dataset = Dataset(max_len, "./carl_carp_parl/")
